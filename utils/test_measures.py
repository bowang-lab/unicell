...
def eval_img_batch(y_gt, y_pred, threshold=0.5, res_format='DataFrame'):
    ...
    for i in tqdm(range(num)):#y_gt.shape[0]
        ...
        seg_i, _, _ = segmentation.relabel_sequential(seg_i)
        cell_true_num = np.max(gt_i)
        cell_pred_num = np.max(seg_i)
        ...
        if tp_i == 0:
            ...
            precision_i = tp_i / cell_pred_num
            recall_i = tp_i / cell_true_num
        ...
        metrics['correct_num(TP)'].append(tp_i)
        metrics['missed_num(FN)'].append(fn_i)    
        metrics['wrong_num(FP)'].append(fp_i)
...
#%% Test function
seg_dict = np.load("/media/jma/sg/junma/UniCell/20220318-nuclei-cell/WholeCell/whole_cell_final/mesmer_segrgb/mesmer_rgbseg.npz")
y_gts = seg_dict['y'][:100,...,0]
y_preds = seg_dict['mesmer_seg'][:100,...,0]
eval_res = eval_img_batch(y_gts, y_preds, threshold=0.5)
