...
def eval_img_list(y_gts, y_preds, threshold=0.5, res_format='DataFrame'):
    ...
    if res_format=='DataFrame':
        ...
        return np.mean(metrics['jaccard'])
#%% Test function
# seg_dict = np.load(r"F:\MetaCell\Results\nuc\segformer_b3\segb3_inst.npz")
# y_gt = seg_dict['y'][:100,...,0]
# y_pred = seg_dict['y_pred'][:100,...,0]
...
import scipy
from tqdm import tqdm
gt_path = '/media/jma/sg/junma/UniCell/AllCellImgs/seg-images/final-seg/labelsTs'
seg_path = '/media/jma/sg/junma/UniCell/AllCellImgs/seg-images/final-seg/mesmer/segmentation'
save_path = '/media/jma/sg/junma/UniCell/AllCellImgs/seg-images/final-seg/'
names = sorted(os.listdir(seg_path))
thre = 0.5

feature_names = ['area_convex', 'equivalent_diameter_area', 'perimeter', 'eccentricity', 'extent']
gt_dict = OrderedDict()
gt_dict['cell'] = list()
gt_dict['label'] = list()
for i in feature_names:
    gt_dict[i] = list()
seg_dict = gt_dict.copy()    
gt_df = pd.DataFrame(gt_dict)
seg_df = pd.DataFrame(seg_dict)
for name in tqdm(names[:304]):
    if name.startswith('Bone') or name.startswith('Seg'):
        ...
        iou = _intersection_over_union(gt, seg)[1:,1:] # shape: np.max(gt)* np.max(seg)
        gt_prop = pd.DataFrame(measure.regionprops_table(gt, properties=['label', 'area_convex', 'equivalent_diameter_area', 'perimeter', 
                                                            'eccentricity', 'extent']))
        cells = [name for i in range(gt_prop.shape[0])]
        gt_prop.insert(0, 'cell', cells)
        
        seg_prop = pd.DataFrame(measure.regionprops_table(seg, properties=['label', 'area_convex', 'equivalent_diameter_area', 'perimeter', 
                                                            'eccentricity', 'extent']))
        cells = [name for i in range(seg_prop.shape[0])]
        seg_prop.insert(0, 'cell', cells)
        
        n_min = min(iou.shape[0], iou.shape[1])
        costs = -(iou >= thre).astype(float) - iou / (2*n_min)
        ind_true, ind_pred = linear_sum_assignment(costs)
        ind_true = ind_true + 1
        ind_pred = ind_pred + 1
        
        for ind_t, ind_p in zip(ind_true, ind_pred):
            cell_true = gt_prop[gt_prop['label']==ind_t]
            cell_pred = seg_prop[seg_prop['label']==ind_p]
            assert cell_true.shape == cell_pred.shape, name
            
            gt_df = pd.concat([gt_df, cell_true])
            seg_df = pd.concat([seg_df, cell_pred])
        # assert gt_df.shape==seg_df.shape, name


gt_df.to_csv(join(save_path, 'fea_mesmer_gt.csv'), index=False)
seg_df.to_csv(join(save_path, 'fea_mesmer_seg.csv'), index=False)

print(f"{gt_df.shape=}, {seg_df.shape=}")
pears = []
for i in feature_names:
    pear = scipy.stats.pearsonr(gt_df[i], seg_df[i])
    pears.append(pear)
    print(i, pear)
