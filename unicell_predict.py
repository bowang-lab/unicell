...
parser.add_argument('-m', '--model_path', default='./work_dir/b2unet-CellUniverse', help='path where to save models and segmentation results')
...
model = MiT_B2_UNet_MultiHead(in_channels=3, out_channels=args.num_class, regress_class=1, img_size=args.input_size).to(device)

checkpoint = torch.load(join(args.model_path, 'latest_model.pth'))
